...
class VITSModule(LightningModule):
    ...
    def forward(
        self, x: Union[torch.Tensor, str, List[str]], noise_scale: float = 0.667
    ):
        if isinstance(x, torch.Tensor):
            ...
            x = (
                torch.LongTensor(text_to_sequence(x.split()))
                .unsqueeze(0)
                .to(self.device)
            )
    ...
    def _handle_batch(self, batch, train=True):
        ...
        y = slice_segments(
            batch.wav, o.idx_slice * self.hop_length, self.sample_segment_size
        )
        y_hat = o.wav
        ...
        loss_dur = masked_mse_loss(
            o.duration, to_log_scale(batch.duration), mask=o.x_mask
        )
        ...
        loss_g = loss_gen + loss_dur + loss_mel + loss_kl + loss_fm
        if train:
    ...
    def configure_optimizers(self):
        ...
        scheduler_g = optim.lr_scheduler.ExponentialLR(
            optimizer_g, **self.params.scheduler_g
        )
        scheduler_d = optim.lr_scheduler.ExponentialLR(
            optimizer_d, **self.params.scheduler_d
        )
